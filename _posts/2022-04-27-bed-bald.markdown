...
$$
\begin{aligned}
I(\xi) \ge \E_{p(\theta_0)p(y\mid \theta_0,\xi)p(\theta_1)\dots p(\theta_L)}\left[ \log \frac{p(y\mid \theta_0,\xi)}{\frac{1}{L+1} \sum_{\ell=0}^L p(y\mid \theta_\ell,\xi)} \right]
\end{aligned}
$$
...
\begin{aligned}
\hat{I}_\text{PCE-RB}(\xi) &= \frac{1}{M} \sum_{m=1}^M \sum_c p(y=c\mid \theta_m,\xi) \log \frac{p(y=c\mid \theta_{m},\xi)}{\frac{1}{M}\sum_{\ell=1}^M p(y=c\mid \theta_{\ell},\xi)  } \\
\begin{split}&= \frac{1}{M} \sum_{m=1}^M \sum_c p(y=c\mid \theta_m,\xi) \log p(y=c\mid \theta_{m},\xi) \\
& \qquad - \frac{1}{M} \sum_{m=1}^M \sum_c p(y=c\mid \theta_m,\xi) \log\left(\frac{1}{M}\sum_{\ell=1}^M p(y=c\mid \theta_{\ell},\xi)  \right) \end{split}\\
