...
$$
\begin{aligned}
I(\xi) = \mathbb{E}_{p(\theta)p(y|\theta,\xi)}\left[H[p(\theta)] - H[p(\theta|y,\xi)]\right]
\end{aligned}
$$
...
One of the computational challenges inherent in estimating mutual information directly is that it involves repeated estimation of posterior distributions $$p(\theta|y,\xi)$$ for different simulated observations $$y$$.
...
\end{aligned}
$$
where $$y_m,\theta_m \sim p(y,\theta|\xi)$$. (To check the expectation of this version matches the PCE bound with $$L=M-1$$, we simply move the $$\mathbb{E}$$ sign inside of the summation.)
...
This proves that $$\mathbb{E}[DBALD(\xi)] \le I(\xi)$$.
We also found a "cousin" of the BALD score that is an *upper* bound on mutual information---it could be useful as a diagnostic.
